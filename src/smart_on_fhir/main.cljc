(ns smart-on-fhir.main
  "Kotodama WASM entrypoint for the Smart_on_fhir clean-room actor (L4) — Clojure port.
  ...
  facts (`smart_on_fhir.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "smart_on_fhir")
(def tier "L4")
...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "smartonf_pat"
   ...
   {:entity "Observation"  :plural "observations"  :id-prefix "smartonf_obs"
    :fields [:resourceType :status :effectiveDateTime :valueString :issued]  :required [:resourceType]
    :coerce {}  :refs {}}
   {:entity "Encounter"  :plural "encounters"  :id-prefix "smartonf_enc"
    :fields [:resourceType :status :serviceType :period]  :required [:resourceType]
    :coerce {}  :refs {}}
   {:entity "MedicationRequest"  :plural "medicationrequests"  :id-prefix "smartonf_med"
    :fields [:resourceType :status :intent :authoredOn]  :required [:resourceType]
    :coerce {}  :refs {}}
   {:entity "AllergyIntolerance"  :plural "allergyintolerances"  :id-prefix "smartonf_all"
    :fields [:resourceType :clinicalStatus :criticality :recordedDate]  :required [:resourceType]
    :coerce {}  :refs {}}
   {:entity "Condition"  :plural "conditions"  :id-prefix "smartonf_con"
...
(defn emit-facts
  "EAVT facts for one record: {\"smart_on_fhir.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "smart_on_fhir-compat" :tier tier :entities entities} 200])
...
(defn start! [] :smart_on_fhir-compat/ready)
